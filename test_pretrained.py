...
spatial_first_weight = tf.get_default_graph().get_tensor_by_name('resnet_v1_50/block1/unit_1/bottleneck_v1/conv2/weights:0')
...
for img_name in os.listdir('./images'):
  ...
  for i in range(8):
    for j in range(8):
      ...
      plt.matshow(weight_img[:, :, 32, j + i*8], vmin=weight_img[:, :, 32, j + i*8].min(), vmax=weight_img[:, :, 32, j + i*8].max(), interpolation='none')
      plt.figure(2)
      plt.subplot(8, 8, j + i*8 + 1)
      plt.imshow(filter_img[0, :, :, j + i*8])
  plt.show()
